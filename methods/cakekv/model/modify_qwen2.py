...
def qwen2_attn_forward_cake(
    ...
    if self.config.prefill[self.layer_idx]:
        ...
        if torch.tensor(tmp_attn_weights).isinf().any():
            tmp_attn_weights = torch.matmul(query_states[..., -self.config.window_size[self.layer_idx]:, :].to(torch.float32), key_states.transpose(2, 3).to(torch.float32)) / math.sqrt(self.head_dim)
            tmp_attn_weights = tmp_attn_weights.to(query_states.dtype)
        if q_len !=1:
        ...
        # print(f"kv cache shape {past_key_value.key_cache[self.layer_idx].shape} ")
