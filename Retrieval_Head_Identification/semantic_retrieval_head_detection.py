...
and https://github.com/nightdessert/Retrieval_Head
...
class LLMNeedleHaystackTester:
    ...
    def retrieval_calculate(self, attention_maxtrix,retrieval_score, inp, step_token):
        for i in range(self.needle_start, self.needle_end):
            if inp.item()==self.prompt_ids[i].item():
                for layer_idx in range(self.layer_num):
                    for head_idx in range(self.head_num):
                        retrieval_score[layer_idx][head_idx][0] += attention_maxtrix[layer_idx][0][head_idx][-1][self.needle_start:self.needle_end].sum().item()
    ...
    def evaluate_and_log(self, context_length, depth_percent):
        ...
        if self.save_contexts:
            ...
            if not os.path.exists(f'contexts/{self.model_version}_SRH'):
                os.makedirs(f'contexts/{self.model_version}_SRH')

            with open(f'contexts/{self.model_version}_SRH/{context_file_location}_context.txt', 'w') as f:
                f.write(context)
            
        if self.save_results:
            # Save the context to file for retesting
            if not os.path.exists(f'results/graph/{self.model_version}_SRH'):
                os.makedirs(f'results/graph/{self.model_version}_SRH')
            
            # Save the result to file for retesting
            p = f'results/graph/{self.model_version}_SRH/{context_file_location}_results.json'
    ...
    def result_exists(self, context_length, depth_percent):
        ...
        results_dir = 'results/' + self.model_version +"_SRH"
    ...
    def start_test(self, args):
        for ni in range(len(self.needle_list)):
            ...
            self.run_test(args)
        if os.path.exists(f"head_score/{self.model_version}_SRH.json"):
            with open(f"./head_score/{self.model_version}_SRH.json", "r") as file:
                head_counter = json.loads(file.readline())
            for k,v in head_counter.items():
                self.head_counter[k] += v
        with open(f"head_score/{self.model_version}_SRH.json", 'w') as f:
