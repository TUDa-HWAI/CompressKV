        ...
        with torch.no_grad():
            ...
            for step_i in range(decode_len):
                ...
                outputs = self.model_to_test(input_ids=inp, past_key_values=past_kv, use_cache=True, \
                    output_attentions=False, block_list=block_list, attn_mode="flash")
    ...
    def evaluate_and_log(self, context_length, depth_percent):
        ...
        if self.mask_topk > 0:
            ...
            if self.score_path is not None:
                path_tmp = self.score_path.split("/")[-1].replace(".json", "")
                save_name = f"{path_tmp}_block_top{self.mask_topk}"
            else:
        ...
        if self.model_version in [ "Qwen1.5-14B-Chat"]:
            prompt = [
...
if __name__ == "__main__":
    ...
    parser.add_argument('--score_path',type=str, default=None,help="path of the score")
    parser.add_argument("--context_lengths", nargs='+', type=int, default=None,
                    help='Explicitly specify context lengths to sweep')
